...
;
; Note: COND ... ENDIFS is equivalent to COND ... THENS which is Forth due to Wil Baden.
...
; Used in place of ELSE for short circuit AND. Resolves multiple counted _IFs.
...
_CASE   MACRO
									LSTOUT-
		_COND			 			; Push an _OF-count of zero onto the control flow stack
									LSTOUT+
									ENDM

_OF		MACRO cond
									LSTOUT-
		_AND_IF cond				; Assemble an _IF while incrementing the _OF-count
									LSTOUT+
									ENDM

_OF_EQ	MACRO src, dest			; src is usually #N, dest can be Rn, X(Rn), &ADDR, ADDR
									; Assemble a word comparison
									LSTOUT+
		cmp		src, dest
									LSTOUT-
